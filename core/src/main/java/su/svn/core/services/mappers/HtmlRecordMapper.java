...
public interface HtmlRecordMapper extends TagMapper {
    ...
    @Mapping(source = "value", target = "html")
    ResourceHtmlRecord toResource(TextRecord record);

    @Mapping(target = "id", ignore = true)
    @Mapping(target = "userName", ignore = true)
    @Mapping(target = "refreshAt", ignore = true)
    ResourceHtmlRecord toResource(NewHtmlRecord record);

    @Mapping(target = "userName", ignore = true)
    ResourceHtmlRecord toResource(UpdateHtmlRecord record);
    ...
    @Mapping(source = "html", target = "value")
    TextRecord toEntity(ResourceHtmlRecord record);
}
